        ...
        static void Main(string[] args)
        {
            ...
            for (var i = 0; i < num; i++)
            {
                ...
                if (match.Success)
                {
                    ...
                    {
                        dict.Add(type, new SortedDictionary<string, int[]>());
                    }
            ...
            {
                Console.WriteLine("{0}::({1:F}/{2:f}/{3:f})", outerPair.Key, outerPair.Value.Select(x => x.Value[0]).Average(), outerPair.Value.Select(x => x.Value[1]).Average(), outerPair.Value.Select(x => x.Value[2]).Average());
