namespace _06.Zipping_Sliced_Files
{
    ...
    class Startup
    {
        static void Main()
        {
            ...
            SliceAndCompress(name, destination, parts);
            AssembleAndDecompress(zips, resultDestination, "mp3");
        }
        ...
        static void SliceAndCompress(string sourceFile, string destinationDirectory, int parts)
        {
            ...
            using (var file = new FileStream(destinationDirectory + "//" + sourceFile, FileMode.Open))
            {
                ...
                {
                    using (var newFile = new FileStream(destinationDirectory + "//result files//part" + (i + 1) + ".gz",
                        FileMode.Create))
                    {
        ...
        static void AssembleAndDecompress(List<string> files, string destinationDirectory, string extension)
        {
